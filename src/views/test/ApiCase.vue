  ...
  <div class="dashboard-container">
          ...
          <el-card class="box-card">
            ...
            >
              ...
              >
                ...
                <el-form-item>
                  <router-link
                    :to="{ name: 'API接口', query: {project_id: project_id, case_id: case_id}}"
                ...
                <el-form-item>
                  <el-button
                    type="primary"
                    size="medium"
                    @click.native="loadSwaggerApi = true"
                  >导入接口</el-button>
                  <el-button
                    type="danger"
                    size="medium"
            ...
            <el-dialog
              title="修改所属分组"
              :visible.sync="updateGroupFormVisible"
            ...
            >
              ...
              >
                ...
                >
                  <el-select
                    v-model="updateGroupForm.firstGroup"
                    placeholder="请选择"
                  >
                    <el-option
              ...
              >
                ...
                <el-button
                  type="primary"
                  @click.native="updateGroupSubmit"
    ...
    >
      ...
      >
        ...
        <el-button
          type="danger"
          @click.native="editGroupSubmit(1)"
          :loading="addGroupLoading"
        >删除</el-button>
        <el-button
          type="primary"
          @click.native="editGroupSubmit(0)"
          :loading="addGroupLoading"
        >提交</el-button>
      </div>
    </el-dialog>
...
export default {
      ...
      addGroupFormRules: {
        groupname: [
          { required: true, message: "请输入分组名称", trigger: "blur" },
          {
            min: 2,
            max: 20,
            message: "请输入长度在 1 到 15 个字符的分组名称",
            trigger: "blur"
          }
        ]
      },
      ...
      editFirstGroupFormRules: {
        secondFirstGroup: [
          { required: true, message: "请输入分组名称", trigger: "blur" },
          {
            min: 2,
            max: 20,
            message: "请输入长度在 2 到 20 个字符的分组名称",
            trigger: "blur"
          }
        ]
      },
  ...
  computed: {
    filterApilist() {
      return this.apilist.filter(
        item =>
          !this.apiName ||
          item.name.toLowerCase().includes(this.apiName.toLowerCase())
      );
    }
  },
  watch: {
  ...
  methods: {
    ...
    getProjectList() {
      ...
      // // , this.project_id
      this.$axios
        .post("/case/list", this.project_id)
        .then(response => {
          if (response.data.status === "SUCCESS") {
